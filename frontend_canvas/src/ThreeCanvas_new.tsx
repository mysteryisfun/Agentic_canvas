...
const ThreeCanvas: React.FC<ThreeCanvasProps> = ({ onCameraUpdate }) => {
  ...
  const animateCamera = useCallback((target: { x: number; y: number; z: number }, zoom: number) => {
    ...
    const endPosition = new THREE.Vector3(target.x, target.y, target.z + (2.5 / zoom));
  ...
  useEffect(() => {
    ...
    const createPlanet = (name: string, radius: number, textureUrl: string, distance: number, revolutionSpeed: number, rotationSpeed: number) => {
      ...
      // Orbit
      const orbitGeometry = new THREE.RingGeometry(distance - 0.01, distance + 0.01, 256);
      const orbitMaterial = new THREE.MeshBasicMaterial({ color: 0x333333, side: THREE.DoubleSide, transparent: true, opacity: 0.3 });
